    ...
    public class DdsTexture
    {
        ...
        public unsafe DdsTexture(byte[] fileData, int byteOffset = 0)
        {
            fixed (byte* pFileData = fileData)
            {
                ...
                if (!FlagsHelper.HasFlag(pHeader->Caps, Caps.Texture))
                    throw new InvalidDataException("Required Caps.Texture flag is missing");

                if (FlagsHelper.HasFlag(pHeader->Flags, HeaderFlags.MipMapCount) ^ FlagsHelper.HasFlag(pHeader->Caps, Caps.MipMap))
                ...
                Depth = FlagsHelper.HasFlag(pHeader->Flags, HeaderFlags.Depth) ? (int)pHeader->Depth : 1;
                ...
                if (pHeader->PixelFormat.FourCC != Helper.DX10)
                {
                    ...
                    CalculateMipInfos(
                      FlagsHelper.HasFlag(pHeader->Flags, HeaderFlags.MipMapCount) ? (int)pHeader->MipMapCount : -1,
                      FlagsHelper.HasFlag(pHeader->Flags, HeaderFlags.Pitch) ? (int)pHeader->LinearSize : -1,
                      FlagsHelper.HasFlag(pHeader->Flags, HeaderFlags.LinearSize) ? (int)pHeader->LinearSize : -1,
                      out chainSize);

                    if (!(FlagsHelper.HasFlag(pHeader->Caps, Caps.Complex) && (FlagsHelper.HasFlag(pHeader->Caps2, Caps2.CubeMap))))
                    {
                        Data = new byte[1][];
                    }
                    else
                    {
                        ...
                        for (int i = 0; i < 6; i++)
                        {
                            ...
                            if (FlagsHelper.HasFlag(pHeader->Caps2, Helper.CubeMapFaces[i]))
                            {
