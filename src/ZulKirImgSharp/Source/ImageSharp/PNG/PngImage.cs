    ...
    public class PngImage
    {
        ...
        public unsafe PngImage(byte[] fileData, int byteOffset = 0)
        {
            fixed (byte* pFileData = fileData)
            {
                ...
                if ((pHeader->LengthFlipped) != Header.DataLength)
                    throw new InvalidDataException("IHDR length must be exactly 13 bytes");

                Width = (int)(ExtensionMethods.FlipEndianness(pHeader->WidthFlipped));
                Height = (int)(ExtensionMethods.FlipEndianness(pHeader->HeightFlipped));
                ...
                while (!endFound)
                {
                    ...
                    int length = (int)(ExtensionMethods.FlipEndianness(pChunkBeginning->LengthFlipped));
                    ...
                    uint crc = ExtensionMethods.FlipEndianness((*(uint*)p));
                    p += 4;
